...
const AddPesanan = () => {
  ...
  useEffect(() => {
    dispatch(setPageTitle('Tambah Pesanan'));
    fetchPelanggan();
    fetchBarangMentah();
    fetchProduk();
  }, [dispatch]);
  ...
  const handleStatusWarning = async (status: string) => {
    // Jika status adalah 'Dikirim' atau 'Selesai', tampilkan warning
    if (status === 'Dikirim' || status === 'Selesai') {
      ...
      const result = await Swal.fire({
        ...
        html: `
          <div class="text-left">
            <p class="mb-3"><strong>Status "${status}" akan mengurangi inventaris:</strong></p>
    ...
    return true; // For other statuses, proceed without confirmation
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const statusConfirmed = await handleStatusWarning(formData.status_pemesanan);
    if (!statusConfirmed) {
    ...
    for (let i = 0; i < formData.detail_barang.length; i++) {
      ...
      if (item.jenis_barang === 'barang_mentah') {
        ...
        if (barangMentah && item.berat > barangMentah.berat_mentah) {
          Swal.fire('Error', `Berat pada baris ${i + 1} tidak boleh melebihi ${barangMentah.berat_mentah * 1000} gram (tersedia)`, 'error');
          return;
        }
      }
    ...
    try {
      ...
      const response = await fetch(buildApiUrl('pesanan'), {
        method: 'POST',
        headers: getAuthHeaders(),
      ...
      if (response.ok) {
        ...
        if (errorData.message && errorData.message.includes('Stok tidak mencukupi')) {
          ...
          Swal.fire('Error', errorData.message || 'Gagal membuat pesanan', 'error');
        }
      }
    } catch (error) {
      console.error('Error creating pesanan:', error);
      Swal.fire('Error', 'Terjadi kesalahan saat membuat pesanan', 'error');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex justify-between flex-wrap px-4">
          <div className="mb-6 lg:w-1/2 w-full">
            <div className="flex items-center text-black dark:text-white shrink-0">
              <h2 className="text-xl font-semibold">Tambah Pesanan</h2>
            </div>
          </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-6">
                    ...
                    <div className="md:col-span-2">
                      ...
                      {process.env.NODE_ENV === 'development' && (
                        <small className="text-xs text-gray-500">
                          {item.jenis_barang === 'barang_mentah' 
                            ? `Barang Mentah: ${barangMentahList.length} items`
                            : `Produk: ${produkList.length} items`
                          }
                        </small>
                      )}
                    </div>
                    ...
                    {item.jenis_barang === 'barang_mentah' && (
                      <div>
                        ...
                        {item.id_barang_mentah && (() => {
                          const barangMentah = barangMentahList.find(b => b.id_barangmentah === item.id_barang_mentah);
                          return barangMentah && (item.berat || 0) > barangMentah.berat_mentah ? (
                    ...
                    {item.jenis_barang === 'produk' && (
                      <div>
                        ...
                        {item.id_produk && (() => {
                          const produk = produkList.find(p => p.id_produk_detail === item.id_produk);
                          return produk && (item.stok || 0) > produk.stok ? (
                            <small className="text-red-500">
