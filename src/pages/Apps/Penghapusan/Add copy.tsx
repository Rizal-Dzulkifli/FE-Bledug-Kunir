...
const AddPenghapusan = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Tambah Penghapusan'));
    ...
    const fetchBarangDitempatkan = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                Swal.fire('Error', data.message || 'Gagal mengambil data barang ditempatkan', 'error');
            }
        } catch (error) {
            Swal.fire('Error', 'Terjadi kesalahan saat mengambil data barang ditempatkan', 'error');
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetch('http://localhost:3333/api/penghapusan', {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (response.ok) {
                Swal.fire('Sukses', 'Transaksi penghapusan berhasil ditambahkan', 'success');
                navigate('/penghapusan');
            } else {
                Swal.fire('Error', data.message || 'Gagal menambahkan penghapusan', 'error');
            }
        } catch (error) {
