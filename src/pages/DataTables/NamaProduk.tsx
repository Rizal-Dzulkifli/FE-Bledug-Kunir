...
// Definisikan tipe data untuk NamaProduk
type NamaProduk = {
    id_produk: number;
    nama_produk: string;
    kode_produk: string;
...
const MasterDataNamaProduk = () => {
    const dispatch = useDispatch();
    const tableRef = useRef<HTMLTableElement>(null);
    const dataTableRef = useRef<any>(null);

    useEffect(() => {
        dispatch(setPageTitle('Master Data Nama Produk'));
        ambilDaftarNamaProduk();
    }, []);

    const [addModal, setAddModal] = useState(false);
    const [search, setSearch] = useState('');
    const [isSearching, setIsSearching] = useState(false);
    const [params, setParams] = useState<NamaProduk>({
        id_produk: 0,
        nama_produk: '',
        kode_produk: '',
    });

    const [daftarNamaProduk, setDaftarNamaProduk] = useState<NamaProduk[]>([]);
    const [dataAwal, setDataAwal] = useState<NamaProduk[]>([]);
    ...
    // Ambil daftar nama produk dari API
    const ambilDaftarNamaProduk = async () => {
        ...
        try {
            ...
            console.log('Processed Data:', namaProdukData); // Debug log
            setDaftarNamaProduk(namaProdukData);
            setDataAwal(namaProdukData);
        } catch (error) {
            console.error('Error mengambil nama produk:', error);
            Swal.fire('Error', 'Tidak dapat mengambil data nama produk', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        ...
        if (tableRef.current) {
            ...
            setTimeout(() => {
                const searchWrapper = document.getElementById('produk-search-wrapper');
    ...
    // Function untuk generate kode produk otomatis
    const generateKodeProduk = async (namaProduk: string) => {
        ...
        try {
            console.log('Generating kode for:', namaProduk); // Debug log
            
            const response = await fetch(buildApiUrl('nama-produk/generate-kode'), {
                method: 'POST',
                headers: getAuthHeaders(),
                body: JSON.stringify({
            ...
            if (response.ok) {
                const generatedKode = data.kode_produk || '';
                ...
                const fallbackKode = generateKodeFallback(namaProduk);
    ...
    const generateKodeFallback = (namaProduk: string): string => {
        const words = namaProduk.toUpperCase().trim().split(' ').filter(word => word.length > 0);
        let kode = '';
    ...
    // Simpan atau perbarui data nama produk
    const simpanData = async () => {
        if (!params.nama_produk.trim()) {
        ...
        try {
            ...
            let kodeProduk = params.kode_produk?.trim();
            if (!params.id_produk && !kodeProduk) {
                kodeProduk = await generateKodeProduk(params.nama_produk);
                console.log('Generated kode:', kodeProduk); // Debug log
            }

            const method = params.id_produk ? 'PUT' : 'POST';
            const url = params.id_produk
                ? buildApiUrl(`nama-produk/${params.id_produk}`)
                : buildApiUrl('nama-produk');

            const requestBody: any = {
                nama_produk: params.nama_produk.trim(),
            };

            // Include kode_produk only if it's provided
            if (kodeProduk) {
                requestBody.kode_produk = kodeProduk;
            }
            ...
            if (!response.ok) {
                ...
                if (responseData.suggested_kode) {
                    ...
                    setParams(prev => ({ ...prev, kode_produk: responseData.suggested_kode }));
                ...
                throw new Error(responseData.message || `Gagal ${params.id_produk ? 'memperbarui' : 'menambah'} data`);
            }

            // Tampilkan pesan sukses dengan informasi tambahan
            let successMessage = `Data berhasil ${params.id_produk ? 'diperbarui' : 'ditambahkan'}.`;
            if (responseData.updated_related_codes) {
                successMessage += ' Kode pada produk terkait juga telah diperbarui.';
            }
            tampilkanPesan(successMessage);
            setAddModal(false);
            setParams({ id_produk: 0, nama_produk: '', kode_produk: '' });

            // Refresh data
            await ambilDaftarNamaProduk();
        } catch (error: any) {
            console.error('Error saving data:', error);
            tampilkanPesan(error.message || `Gagal ${params.id_produk ? 'memperbarui' : 'menambah'} data`, 'error');
        }
    };

    // Edit data nama produk
    const editData = (item: NamaProduk) => {
        setParams(item);
        setAddModal(true);
    };

    // Hapus data nama produk
    const hapusData = (id_produk: number) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await fetch(buildApiUrl(`nama-produk/${id_produk}`), {
                        method: 'DELETE',
                        headers: getAuthHeaders(),
                    });
    ...
    const handleNamaChange = async (value: string) => {
        setParams({ ...params, nama_produk: value });
        
        // Auto generate kode hanya untuk data baru atau jika kode belum diisi manual
        if (!params.id_produk && value.trim()) {
            const kodeGenerated = await generateKodeProduk(value);
            if (kodeGenerated) {
                setParams(prev => ({ ...prev, nama_produk: value, kode_produk: kodeGenerated }));
            }
        } else if (params.id_produk && value.trim()) {
    ...
    return (
        <div className="space-y-6">
            <div className="panel flex items-center overflow-x-auto whitespace-nowrap p-3 text-primary">
                ...
                <span className="ltr:mr-3 rtl:ml-3">Dokumentasi: Master Data Nama Produk untuk mengelola kategori produk jadi dengan auto-generate kode produk dan integrasi sistem produksi</span>
            </div>
            <div className="flex justify-between items-center mb-5">
                ...
                <div id="produk-search-wrapper" className="relative">
                    <input
                        type="text"
                ...
                <div className="overflow-x-auto">
                    <div className="table-responsive mb-5">
                        {loading ? (
                            ...
                            <table ref={tableRef} id="namaProdukTable" className="table-striped w-full">
                                <thead>
                                    <tr>
                                        <th className="px-6 py-3 border-b-2 font-semibold text-black dark:text-white whitespace-nowrap">Kode Produk</th>
                                        <th className="px-6 py-3 border-b-2 font-semibold text-black dark:text-white whitespace-nowrap">Nama Produk</th>
                                        <th className="px-4 py-3 border-b-2 font-semibold text-black dark:text-white text-center whitespace-nowrap">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                {daftarNamaProduk.map((record, index) => (
                                    <tr key={`${record.id_produk}-${index}`} className="hover:bg-gray-100 dark:hover:bg-gray-800">
                                        <td className="px-6 py-4 font-mono text-sm">{record.kode_produk}</td>
                                        <td className="px-6 py-4">{record.nama_produk}</td>
                                        <td className="px-4 py-2 text-center">
                                            <div className="flex gap-2 justify-center">
                                                ...
                                                <button className="btn btn-sm btn-outline-danger" onClick={() => hapusData(record.id_produk)}>
                            ...
                            <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                                <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                                    ...
                                    <div className="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]">
                                        {params.id_produk ? 'Edit Nama Produk' : 'Tambah Nama Produk'}
                                    </div>
                                    <div className="p-5">
                                        <form>
                                            <div className="mb-5">
                                            ...
                                            <div className="mb-5">
                                                ...
                                                <input 
                                                    ...
                                                    className="form-input" 
                                                    value={params.kode_produk} 
                                                    onChange={(e) => setParams({ ...params, kode_produk: e.target.value.toUpperCase() })} 
                                                />
                                                <small className="text-gray-500">Kosongkan untuk generate otomatis dari nama produk</small>
                                            </div>
                                            <div className="flex justify-end items-center mt-8">
                                                ...
                                                <button type="button" className="btn btn-primary ltr:ml-4 rtl:mr-4" onClick={simpanData}>
                                                    {params.id_produk ? 'Perbarui' : 'Tambah'}
                                                </button>
                                            </div>
                                        </form>
                                    </div>
...
export default MasterDataNamaProduk;